#include <iostream>
#include <fstream>
#include "/ee259/tools/pro_2/sample_p0.h" // implementation of base class
...
{
	int i, j;
	output_file_p2 << "++ P1 START ++" << endl;
	output_file_p2 << "++ P1 OUTPUT FROM STUDENT_GRADE CONSTRUCTOR:" << endl;	
	p = y;
	for(i = 0; i < n; i++)
	{
		for(j = 0; j < p; j++)
	...
	output_file_p2 << "++ P1 AN OBJECT OF STUDENT_GRADE IS CREATED WITH " << x << " STUDENTS AND " 
		<< y << " EXAMS EACH." << endl;
	output_file_p2 << "++ P1 END ++" << endl;
}
...
{
	output_file_p2 << "++ P1 START ++" << endl;
	output_file_p2 << "++ P1 OUTPUT FROM LIST_IDS METHOD:" << endl;	
	LIST_ID(1);
	output_file_p2 << "++ P1 END ++" << endl;
}
...
STUDENT_GRADE::LIST_GRADE(int x)
{
	...
	FOUND = 0;
	output_file_p2 << "++ P1 START ++" << endl;
	output_file_p2 << "++ P1 OUTPUT FROM LIST_GRADE METHOD:" << endl;

	if (x < 0 || x >=10000)
	{
		output_file_p2 << "++ P1 INPUT ERROR" << endl;
	}
	else
	{
	...
	//cout << " FOUND is " << FOUND << " FOUND_POS IS " << FOUND_POS << endl;
		if (FOUND == 1)
		{
			output_file_p2 << "++ P1 GRADES FOR STUDENT " << x << ":" << endl;
			for(j = 0; j < p; j++)
			{
				output_file_p2 << "++ P1 GRADE FOR EXAM " << j << " IS: " 
					<< grades[FOUND_POS][j] << endl;
			}
		}
		else
		{
			output_file_p2 << "++ P1 NO SUCH STUDENT" << endl;
		}
	}// end else
	output_file_p2 << "++ P1 END ++" << endl;
}// end method
...
STUDENT_GRADE::LIST_RANGE(int x)
{
	...
	output_file_p2 << "++ P1 START ++" << endl;
	output_file_p2 << "++ P1 OUTPUT FROM LIST_RANGE METHOD:" << endl;
	...
	else
	{
		...
		range = max - min;
		output_file_p2 << "++ P1 RANGE FOR EXAM " << x << " IS " << range << "." << endl;
	} // end else
	output_file_p2 << "++ P1 END ++" << endl;
}// end method
