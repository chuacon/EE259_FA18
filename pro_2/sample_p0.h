...
ofstream output_file_p2("out.2.txt", ios::out);
...
STUDENT_ID::STUDENT_ID(int x)
{
	...
	output_file_p2 << "+ P0 START +" << endl;
	output_file_p2 << "+ P0 OUTPUT FROM STUDENT_ID CONSTRUCTOR:" << endl;
	output_file_p2 << "+ P0 INSTANTIATED AN OBJECT OF CLASS STUDENT_ID WITH " << n << " STUDENT IDS." << endl;
	output_file_p2 << "+ P0 END +" << endl;
}
...
STUDENT_ID::LIST_ID(int x)
{
	...
	output_file_p2 << "+ P0 START +" << endl;
	output_file_p2 << "+ P0 OUTPUT FROM LIST_ID METHOD:" << endl;

	if(x == 1)
	{
		output_file_p2 << "+ P0 STUDENT IDS FOR " << n << " STUDENTS:" << endl;
		for(i = 0; i < n; i++)
		{
			output_file_p2 << "+ P0 " << ids[i] << endl;	
		}
	}
	else if (x == -1)
	{
		output_file_p2 << "+ P0 STUDENT IDS FOR " << n << " STUDENTS IN REVERSE ORDER:" << endl;
		for(i = n-1; i >=0; i--)
		{
			output_file_p2 << "+ P0 " << ids[i] << endl;	
		}
	...
	{
		output_file_p2 << "+ P0 INPUT ERROR." << endl;
	}
	output_file_p2 << "+ P0 END +" << endl;
}
