...
/*
  ...
  */
    ...
    out_f<<"LEAST_SQUARE_FIT RESULT:"<<endl;
    out_f<<"USING INHERITANCE:"<<endl;
  	out_f<<"MATCHING FUNCTION IS Y  = "<< setprecision(2)<<setiosflags(ios::fixed | ios::showpoint) << m << "*"<<" X + "<<b<<endl;

   /* out_f.setf(ios::fixed);
    out_f.setf(ios::showpoint);
    out_f.precision(2);
    out_f<<"MATCHING FUNCTION IS Y  = "<<m<<"*"<<"X + "<<b<<endl;
*/
    for(i=0;i<np;i++)
    {
  ...
  /*
    ...
    out_f.precision(3);
    */
    out_f<<"THE ERROR FROM LS_FIT_BY_ME METHOD IS "<<setprecision(3)<<setiosflags(ios::fixed | ios::showpoint) << error <<endl;
//   system(" rm -f out.6 && touch out.6");
